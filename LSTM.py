...
# 报错，现在keras完全置于tf模块中，这个要从tensoflow根模块导入，修改为：
# 查错来源https://blog.csdn.net/weixin_39754630/article/details/118097037
...
# 报错，查错来源https://blog.csdn.net/weixin_43582101/article/details/117079573
...
from tensorflow_core.python.keras.layers import Conv1D, MaxPooling1D, Dropout, Flatten, Dense, Bidirectional, LSTM
...
def three_CNN_LSTM1(imgName, x_train, y_train, x_val, y_val, x_test, y_test, learning_rate, INPUT_SHAPE, KERNEL_NUMBER, KERNEL_SIZE, LSTM_UNITS):
    model = Sequential()
    ...
    plt.plot(Tuning.history['accuracy'])
    plt.plot(Tuning.history['val_accuracy'])
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Model - Accuracy')
    plt.legend(['Training', 'Validation'], loc='lower right')
    plt.savefig(imgName + '_model_acc.png')
    plt.show()
    ...
    plt.savefig(imgName + '_model_loss.png')
    plt.show()
...
def three_CNN_LSTM2(x_train, y_train, x_val, y_val,
                    x_test1, y_test1, learning_rate, INPUT_SHAPE, KERNEL_NUMBER, KERNEL_SIZE, LSTM_UNITS):
    model = Sequential()
    model.add(Conv1D(KERNEL_NUMBER, kernel_size=KERNEL_SIZE, activation='relu', input_shape=INPUT_SHAPE))
    model.add(MaxPooling1D())
    model.add(Dropout(0.3))
